...
- `/portfolio` (`src/app/portfolio/page.tsx`) — the leadership view: two tabs over the whole registry — **Health** (phase flow with per-phase cycle time, throughput, time-to-decision, what's not moving, gate outcomes, risk mix, owner load) and **Value** (committed vs benefit, where the money sits, KPI attainment, value by function, an outcomes ledger). Its rail answers portfolio questions and can return cards or a Markdown digest; the primary button opens the digest as a document. Reached from a route tab in the tracker's tab row.
